...
def f2(t, q, args):
  #q[0] = x, q[1] = v_x
  # args[0] = k1, args[1] = m, args[2] = k2
  dqdt = np.zeros(2)
  dqdt[0] = q[1]
  dqdt[1] = -args[0]/args[1] * q[0] - args[2] / args[1] * q[0]**3
  return dqdt
...
def get_motion2(k1=1, m=1, k2=1, x0=1, T=20, N=100):
  q0 = np.array([x0, 0.0])
  t0 = 0.0
  
  # Set up the integrator
  integrator = ode(f2).set_integrator('lsoda')
  
  # Set the initial conditions
  integrator.set_initial_value(q0, t0).set_f_params([k1, m, k2])
...
result2 = get_motion2(50.0, 1.0, 1000.0, 0.1, max_time, number_of_points)
plt.plot(result[:,0], result[:,1], label="Simulated linear position")
plt.plot(result[:,0], result[:,2], label="Simulated linear velocity")
plt.plot(result2[:,0], result2[:,1], label="Simulated non-linear position")
plt.plot(result2[:,0], result2[:,2], label="Simulated non-linear velocity")
plt.legend()
plt.xlabel("Times (s)")
plt.ylabel("Velocity (m/s) and postion (m)")
plt.savefig("nonlinearspring.png")
plt.show()
